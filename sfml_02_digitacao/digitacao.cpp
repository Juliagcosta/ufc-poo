...
struct Pincel {
    ...
    Pincel(sf::RenderWindow& window) : window(window) {
        if (!font.loadFromFile("ShareTech-Regular.ttf")){
...
struct Game {
    ...
    Game() : window(sf::VideoMode(800, 600), "Jogo de digitacao"), board(window) {
        this->on_update = [&]() {
            this->draw();
        };
    ...
    void gameover() {
        ...
        pincel.write("Sua pontuacao: ", 400, 400, 20, sf::Color::Black);

        pincel.write("-- Pressione a tecla espaco para fechar o jogo --", 100, 500, 30, sf::Color::White);
